...
if __name__ == "__main__":
    mods_file = "resourcepacks"
    if os.path.exists(mods_file):
        shutil.rmtree(mods_file)
        command = "cp -r custom/resourcepacks resourcepacks"
        result = subprocess.run(command, shell=True, capture_output=True, text=True)
    if not os.path.exists(mods_file):
        os.makedirs(mods_file)
    print("================================================")
    print("Getting resourcepacks from curseforge")
    print("================================================")
    links = get_download_links("download_lists/resourcepacks.org", "[curseforge_download_link]")
    for link in links:
        download_curseforge(link, mods_file)

    print("================================================")
    print("Getting resourcepacks from modrinth")
    print("================================================")
    links = get_download_links("download_lists/resourcepacks.org", "[modrinth_download_link]")
    for link in links:
        download(link, mods_file)
    command = "cp -r resourcepacks custom_configuration/config/paxi/resourcepacks"
    result = subprocess.run(command, shell=True, capture_output=True, text=True)
